...
@section('content')
    ...
    @endforeach
    @endif
    <form action="{{ route('edit_products', ['id' => $detail->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="text" placeholder="Name" name="name" value="{{ $detail->name }}"><br>


        <input type="text" placeholder="Giá" name="gia" value="{{ $detail->gia }}"><br>

        <input type="text" placeholder="Mô tả" name="mota" value="{{ $detail->mota }}"><br>
